...
def main_calibration():
    ...
    while cap.isOpened():
        ...
        gaussian2 = cv2.GaussianBlur(binary,(5,5),0)

        # gaussianblur gray
        gaussian = cv2.GaussianBlur(gray_otsu, (5, 5), 0)
        ...
        cv2.imshow('gray-gaussian', gaussian1)
        #cv2.imshow('binary-gaussian', gaussian2)
        #cv2.imshow('gray_otsu-gaussian',gaussian)
        #cv2.imshow('gray_otsu',gray_otsu)
        ...
        if ids is not None:
            ...
            ret = aruco.estimatePoseSingleMarkers(corners, 1, camera_matrix, dist_matrix)
            ...
            print(str_position)
            cv2.putText(frame, 'str_position:' + str_position + str('cm'), (0, 150), font, 1, (0, 255, 0), 2, cv2.LINE_AA)
            #            cv2.LINE_AA)
            #print(rvec)

            rvec2, tvec2, _ = aruco.estimatePoseSingleMarkers(corners, 1, camera_matrix, dist_matrix)
            (rvec2 - tvec2).any()

            #在画面上 标注auruco标签的各轴
            for i in range(rvec2.shape[0]):
                aruco.drawAxis(frame, camera_matrix, dist_matrix, rvec, tvec, 0.3)
                ...
                ml_scale_use.pix_scale(p3[0],p3[1],markersize)
            ...
            #print(rvec)
            #考虑Z轴（蓝色）的角度
            deg=rvec2[0][0][2]/math.pi*180
            #deg=rvec2[0][0][2]/math.pi*180*90/104
            # 旋转矩阵到欧拉角
